...
class RefineEstimate(object):
    ...
    def load_models(self, weight_paths):
        ...
        for i, path in enumerate(weight_paths):
            model = CMRNet(
                self.image_shape, use_feat_from=1, md=4, use_reflectance=False
            )
    ...
    def project_and_crop_pc_into_cam_frame(self, camera_pose):
        ...
        indexes = local_map[:, 1] > -25.0
        indexes = indexes & (local_map[:, 1] < 25.0)
        indexes = indexes & (local_map[:, 0] > -10.0)
        indexes = indexes & (local_map[:, 0] < 100.0)
    ...
    def project_lidar(self, pc, image_shape, shape_pad):
        ...
        """
        uv, depth, points, refl = self.camera_model.project_pytorch(
            pc, image_shape, None
        )
        uv = uv.t().int()
        ...
        depth_img = visibility.depth_image(
            uv, depth, depth_img, uv.shape[0], image_shape[1], image_shape[0]
        )
        depth_img[depth_img == 1000.0] = 0.0
    ...
    def refine_pose_estimate(self, predicted_pose, image):
        ...
        with torch.no_grad():
            for i, model in enumerate(self.models):
                T_predicted, R_predicted = model(
                    processed_image.unsqueeze(0), pc_image.unsqueeze(0)
                )
                ...
                local_pc_in_cam_frame = rotate_forward(
                    local_pc_in_cam_frame, RT_predicted
                )
                pc_image = self.project_lidar(
                    local_pc_in_cam_frame, image_shape, shape_pad
                )
                images.append(out3)
    ...
    def update(self, img_id, current_transform, img_rgb):
        ...
        return gps_pos, cmr_global_transform_estimate, images
...
if __name__ == "__main__":
    ...
    config = {
        "weight_paths": [
            "./CMRNet/checkpoints/iter1.tar",
            "./CMRNet/checkpoints/iter2.tar",
            "./CMRNet/checkpoints/iter3.tar",
        ],
