...
for img_id in np.arange(length_traj):
    img = cv2.imread(
        dataset_image_dir + "00/image_0/" + str(img_id).zfill(6) + ".png", 0
    )
    ...
    # cv2.circle(traj, (draw_x,draw_y), 1, (img_id*255/(length_traj-1),255-img_id*255/(length_traj-1),0), 1)
...
np.save("estimated_trajectory.npy", trajectory)
